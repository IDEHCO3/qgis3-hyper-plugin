...
class GeoJsonFeature(object):
    def __init__(self, json_data, resource_properties):
        ...
        self.properties = json_data.get('properties') and list(json_data['properties'].values()) or None
        self.keys = json_data.get('properties') and list(json_data['properties'].keys()) or None
...
class SimpleJSON(object):
    ...
    def as_qgs_features(self):
        ...
        f.setAttributes(list(self.fields.values()))

        return [f]


class SimpleJSONCollection(object):
    def __init__(self, resource):
